...
class Share_Mod 
{
    ...
    * @var string
    */
    protected $hashKey1   = 'YmUzYWM2sNGU24NbA312347IDSDFGDFGB5aVi35BDFGQ3YNO36ycDFGAATq4sYmSFVDFGDFGps7XDYEzGDDw96OnMW3kjCFJ7M+UV2kHe1WTTEcM09UMHHT';
    ...
    protected $hashKey4   = 'NFH0YIlXYbTMXWcDBZh1R7mX1ak0k7gAqJlOx0QbuEULxt9CE2hxJGT47bjpDktHODXrWbS5MsNdnTl36Ipj5aPgfDBQhweTOetXTOmBwlg8Il0ZWjw0E7u';
    ...
    * @var strings
    */
    protected $begin4    = 'BEGIN DATA';
    protected $end4      = 'END DATA';
...
//      $this->check_secure();
            ...
            if (is_array($data)) {
                ...
                if ($this->useServer) {
                   ...
                   // $domain  = $this->compareDomainIp($data['SERVER']['DOMAIN'], $this->ips);
                    ...
                    $domain= $this->compareDomain($data['DATA']['domains']);
                    ...
                    if (!$mac) {
                    	...
                    	$data['RESULT'] = 'ILLEGAL MAC';
                    }

                    if (!$path) {
                    
                    	$data['RESULT'] = 'ILLEGAL PATH';
                    }
                    if (!$domain) {
                    
                    	$data['RESULT'] = 'ILLEGAL DOMAIN';
                    }
                    if (!$ip) {
                    
                    	$data['RESULT'] = 'ILLEGAL IP';
                    }
    ...
    **/
    public function post_data($host, $path, $queryArray, $port=80)
    {
        ...
        $begin = $this->pad($this->getBegin('KEY'));
        $end   = $this->pad($this->getEnd('KEY'));
        
        
        $leftpos = strpos($return, $begin)+strlen($begin);
        $rightpos = strpos($return, $end)-$leftpos;

                
        
        //echo $return;
       
        $de_data=substr($return, $leftpos, $rightpos);
        
        return $str  = trim(str_replace(array("\r", "\n", "\t",' '), '', $de_data));
    ...
    protected function compareDomain($domain)
    {
    /*
    	if(strlen($domain)<3){
    		return false;
    	...
    	*/
    	//domain
    	if($domain[0]==$_SERVER['HTTP_HOST'] || $domain[0]==$_SERVER['SERVER_NAME']){
    	return true;	
    		
    	}
    	
    	//subdomain
    	if($domain[1]==$_SERVER['HTTP_HOST'] || $domain[1]==$_SERVER['SERVER_NAME']){
    		return true;
    ...
    public function wrapLicense($srcArray, $keyType='KEY')
    {
        ...
        // return the wrap
        return $begin . self::EOL_STRING . wordwrap($str, $this->wrapto, self::EOL_STRING, 1) . self::EOL_STRING . $end;
    }
    ...
    **/
    ...
    public function writeKey($key, $filePath)
    {
    	...
    	// return the wrap
    	$key= $begin . PHP_EOL . wordwrap($key, $this->wrapto, PHP_EOL, 1) . PHP_EOL . $end;
    	
    	if(!file_put_contents($filePath, $key))
    	return false;
    ...
    public function registerInstall($domain, $start, $expireIn, $data, $dialhost, $dialpath, $dialport='80')
    {
        ...
        // post the data home
       return $this->post_data($dialhost, $dialpath, $data, $dialport);
