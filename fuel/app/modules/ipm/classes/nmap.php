...
class Nmap {
    ...
    public function scan($targets, $with_sudo = false) {
        ...
        if ($ret_var > 0) {
            throw new Nmap_Exception(implode(' ', $out));
        } else {
            foreach ($out as $row) {
                preg_match(
                        '@^Failed to resolve given hostname/IP:\s+(.+)\.\s+Note@', $row, $matches
                );
    ...
    public function parseXMLOutput($output_file = null) {
        ...
        $parse = new Nmap_Parser();
        $parse->setInputFile($output_file);
        $parse->folding = false;

        $res = $parse->parse();
        if (PEAR::isError($res)) {
            throw new Nmap_Exception($res);
        }
    ...
    public function enableOptions($nmap_options) {
        ...
        if ($enable_port_ranges) {
            ...
            if (preg_match($regexp, $port_ranges)) {
                ...
                throw new Nmap_Exception('Port ranges: not valid format.');
            }
        }
